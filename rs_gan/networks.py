...
from rs_gan.gen_resblocks import Block as GenBlock
from rs_gan.disc_resblocks import Block as DiscBlock
from rs_gan.disc_resblocks import OptimizedBlock
...
class ResNetGenerator32(nn.Module):
    ...
    def __init__(self, num_features=256, dim_z=128, channel=3, bottom_width=4,
                 activation=F.relu, num_classes=0, bottleneck=False, norm=True):
        ...
        self.block2 = GenBlock(num_features, num_features,
                            ...
                            num_classes=num_classes, bottleneck=bottleneck, norm=norm) 
...
class SNResNetProjectionDiscriminator32(nn.Module):
    def __init__(self, num_features=256, channel=3, num_classes=0, activation=F.relu, bottleneck=False, norm=True):
        super(SNResNetProjectionDiscriminator32, self).__init__()
        self.num_features = num_features
        self.num_classes = num_classes
        self.activation = activation

        self.block1 = OptimizedBlock(channel, num_features, activation=activation, bottleneck=bottleneck, norm=norm)
        self.block2 = DiscBlock(num_features, num_features,
                            activation=activation, downsample=True, bottleneck=bottleneck, norm=norm)
        ...
        if norm:
            ...
            self.proj = nn.Linear(num_features, 1, bias=False)
        self._initialize()
...
def getGD_SN(structure, dataset, image_size, num_features, ignoreD=False, dim_z=128, bottleneck=False, norm=True):
    leaky_relu = lambda x: F.leaky_relu(x, negative_slope=0.1)
    print("norm:", norm)
    if structure == 'resnet':
        if image_size == 32:
            netG = ResNetGenerator32(num_features=num_features, bottleneck=bottleneck, norm=norm)
            if not ignoreD:
                netD = SNResNetProjectionDiscriminator32(num_features=num_features//2, activation=leaky_relu, bottleneck=bottleneck, norm=norm)
        elif image_size == 48:
            netG = ResNetGenerator48(num_features=num_features, bottleneck=bottleneck, norm=norm)
            if not ignoreD:
                netD = SNResNetProjectionDiscriminator48(num_features=num_features, activation=leaky_relu, bottleneck=bottleneck, norm=norm)
